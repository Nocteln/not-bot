...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if(interaction.options.getString("commande")) {
      command = interaction.client.commands.get(interaction.options.getString("commande"))
    ...
    if(!command) {
      ...
      let embed = new Discord.EmbedBuilder()
            ...
            .setDescription(`Commandes disponibles : \`${interaction.client.commands.size}\` \n Catégories disponibles : \`${categories.length}\``)
            .setTimestamp()
            .setFooter({text: "commandes du bot"})

            await categories.sort().forEach(async cat => {

              let commands = interaction.client.commands.filter(cmd => cmd.cat === cat)
              embed.addFields({name: `${cat}`,value: `${commands.map(cmd => `\`${cmd.data.name}\` : ${cmd.data.description}`).join('\n')}`})
            })
      ...
      .setDescription(`Nom: \`${command.data.name}\`\nDescription :\`${command.data.description}\`\nCatégorie: \`${command.cat}\`\nCommande utilisable en mp : \`${command.data.dm_permission? "oui": command.data.dm_permission === undefined ?"oui" : "non"}\`\nUtilisation: \`${command.uti}\``)
