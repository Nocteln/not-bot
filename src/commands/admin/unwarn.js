...
module.exports = {
  data: new Discord.SlashCommandBuilder()
    .setName("unwarn")
    .setDescription(
      "Permet d'enlever l'avertissement d'un utilisateur dans le serveur"
    )
    ...
    .addUserOption((o) =>
      o
        .setName("membre")
        .setDescription("le membre à unwarn")
        .setRequired(true)
    )
    .addStringOption((o) =>
      o.setName("raison").setDescription("La raison de l'unwarn")
    ),
  cat: "admin",
  ...
  async execute(interaction) {
   ...
   if(member.id === interaction.guild.ownerId) return interaction.reply({embeds: [embedr("Red", ":x: erreur", "vous ne pouvez pas vous warn le propriétaire du serveur!")], ephemeral: true});
    ...
    interaction.reply({embeds: [embedr("Green", `:white_check_mark: succes!`, "Vous avez bien warn **" + member.user.tag + "**!\n ID du warn : \`" + id + "\`")], ephemeral: true});
  },
};
